...
class AppStore extends Component {
  state = {activeTab: tabsList[0].tabId, initialSearch: ''}

  searchValue = event => this.setState({initialSearch: event.target.value})
  ...
  render() {
    const {activeTab, initialSearch} = this.state
    const searchFilter = appsList.filter(each =>
      each.appName.toLowerCase().includes(initialSearch.toLowerCase()),
    )
    const tabFilter = searchFilter.filter(each => each.category === activeTab)
    console.log(activeTab, initialSearch, appsList)
    ...
    return (
      <div className="main_container">
        ...
        <ul className="app_item_container">
          {tabFilter.map(each => (
            <AppItem key={each.appId} myappList={each} />
          ))}
        </ul>
      </div>
    )
